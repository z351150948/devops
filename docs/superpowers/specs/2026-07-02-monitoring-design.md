...
| D-2 | 数据库支持范围 | **MySQL 8 + Redis 7 + MongoDB** | 项目已装 pymysql / pymongo，需补 redis-py 依赖（**注**：redis-py 是否已装在 `requirements.txt` 需在 plan 任务 1 确认；如未装且不允许装新 pip 包，D-2 降级为 MySQL + MongoDB） |
...
> ⚠️ **D-2 风险提示**：redis-py 不在当前 `backend/requirements.txt` 中（B 轮 spec 写作时为 0.0.0 假设）。plan 任务 1 须先确认 redis-py 状态。如未装且内网不能 `pip install`，D-2 降级为「MySQL 8 + MongoDB」（决策变体 D-2b），spec 不动，plan 实施时按实际状态定。
...
- `sqlaudit.DataSource`（`backend/sqlaudit/models.py`）— 数据库连接，含 `connection_params` JSONField
...
- 同样模式，MySQL / Redis / MongoDB 各自响应结构：
  ...
  - **Redis**：`{"status", "duration_ms", "metrics": {"connected_clients": N, "used_memory_mb": N, "used_memory_peak_mb": N, "hit_rate": 0.95, "ops_per_sec": {"get": N, "set": N, "del": N}, "keyspace": {"db0_keys": N}}, "error": null}`
...
| 风险 | 缓解 |
| --- | --- |
| redis-py 不在 requirements.txt | plan 任务 1 先确认；如未装且内网不能装，D-2 降级为 D-2b（仅 MySQL + MongoDB） |
| cmdb.Host ssh_password 明文 | B 轮不引入加密层；spec § 13 留 hotfix TODO |
| 单 host 拉指标超时 | 5s 超时 + 单独 `status: timeout` 字段，不影响其他 host |
| sqlaudit.DataSource connection_params 字段格式不一致 | service 层做 schema 兼容：缺失字段返回 `misconfigured` 而非崩溃 |
| 后端并发过载 | 16 host / 8 并发分批；DB 顺序执行（MySQL/Redis/Mongo 不并发） |
