...
Autotoll DevOps 是一个面向真实运维现场的开源运维智能体平台。它把 **可观测性、事件中心、任务中心、工单审批、容器管理、RBAC** 等平台能力组织成智能体可调用、可审计、可确认的运维工作流。

> **Autotoll DevOps** 是 SxDevOps 的二次发行版（rebrand fork），主要在「品牌外观、Traces 后端默认、Provider 体验」三方面做了定制，源代码能力与上游保持一致。
>
> 看态势、找证据、问系统、确认动作。
...
- 本仓库：[github.com/z351150948/devops](https://github.com/z351150948/devops)
- 技术栈：`Django + Django REST framework + Channels + Vue 3 + Element Plus`
- 开源协议：[Apache License 2.0](LICENSE)

<img src="docs/screenshots/sxdevops-operation-flow.png" alt="Autotoll DevOps 运转逻辑" width="820" />
...
Autotoll DevOps 的目标是把这些碎片化能力收敛成一条链路：**可观测性取证，事件中心复盘，任务中心行动，AIOps 负责理解、规划和结构化输出**。

## 产品定位

Autotoll DevOps 智能体 = **可观测性 + 事件中心 + 任务中心 + AIOps**
...
Autotoll DevOps 在 SxDevOps 上游基础之上做了二次定制，重点包括：

- **品牌外观**：顶部品牌色、Logo 与 `patches/` 中的启动占位截图替换为 Autotoll DevOps 品牌资源。
- **Traces 后端默认**：上游默认 SkyWalking；本仓库默认 Tempo，可通过 `SKYWALKING_ENABLED=1` / `TEMPO_ENABLED=0` / `TRACING_DEFAULT_PROVIDER=skywalking` 切换回 SkyWalking。
- **Provider 体验**：永久禁用 demo provider（migration 0022 + `SXDEVOPS_SEED_DEMO_PROVIDER=0` 默认值），避免误开。
- **Grafana JSONField 兼容**：`observability_views.py` 用 `_coerce()` 强转，幂等解析（修复 MySQL 偶发返回字符串 → 异常）。
- **SkyWalking GraphQL Schema**：`ServiceLayer` 改为 `String`（兼容 OAP 9.x 移除枚举后的版本）。

上游参考与体验站见上方"上游项目"、"上游体验"链接；如果只想跑上游原版体验，把 `patches/` 留空、把 `docker-compose.yml` 里所有 `TEMPO_*`/`TRACING_*` 注释掉即可。
...
Autotoll DevOps 基于 [Apache License 2.0](LICENSE) 开源。Autotoll Devops fork 同样遵循 Apache 2.0。分发或二次开发时请保留项目中的 [NOTICE](NOTICE) 文件与版权声明。
